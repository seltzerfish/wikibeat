...
p = profiles.beat2
song = AudioSegment.from_wav('audio/beat2.wav')
for i, a in enumerate(p['adlib']):
    save_wav(choice(ADLIBS), "a" + str(i))
    print("ad")
    # sleep(2)
    lib = AudioSegment.from_file("a" + str(i) + ".wav")
...
for c_range in p['couplet']:
    ...
    couplet_counter += 1
    save_wav(couplet[0], "c1")
    save_wav(couplet[1], "c2")
    system("ffmpeg -i c1.wav c1.wav -y")
    system("ffmpeg -i c2.wav c2.wav -y")
    l1 = get_duration_wav("c1.wav") * 1000
    l2 = get_duration_wav("c2.wav") * 1000
    if l1 > c_range[0][1] - c_range[0][0]:
        range_len = c_range[0][1] - c_range[0][0]
        speedup = float(l1) / range_len
        stretch("c1.wav", "c1.wav", speedup)
    if l2 > c_range[1][1] - c_range[1][0]:
        range_len = c_range[0][1] - c_range[0][0]
        speedup = float(l2) / range_len
        stretch("c2.wav", "c2.wav", speedup)


    c1_a = AudioSegment.from_file("c1.wav")
    c2_a = AudioSegment.from_file("c2.wav")
    song = song.overlay(c1_a, position=c_range[0][1] - l1)
    song = song.overlay(c2_a, position=c_range[1][1] - l2)


save_wav("Hackathons", "title")
print("tite")
title = AudioSegment.from_file("title" + ".wav") 
song = song.overlay(title, position=p['title'])

song.export('final.wav')
